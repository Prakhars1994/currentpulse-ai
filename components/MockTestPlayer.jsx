...
export default function MockTestPlayer({ exam, testNumber, questions, marksPerCorrect = 1, negativeMarks = 0.25 }) {
    ...
    return (
      <div className="space-y-6">
        <div className="rounded-3xl border border-cyan-400/20 bg-slate-900 p-7">
          ...
          <p className="mt-2 text-slate-300">{result.correct} correct · {result.wrong} wrong · {result.unanswered} unanswered</p>
          <div className="mt-5 flex gap-3"><button onClick={() => { setAnswers({}); setFinished(false); }} className="rounded-xl bg-cyan-400 px-5 py-3 font-black text-slate-950">Retake</button><Link href={`/mock-tests/${exam}`} className="rounded-xl border border-slate-700 px-5 py-3 font-bold">All 10 mocks</Link></div>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          {result.subjects.map(([name, s]) => <div key={name} className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><b>{name}</b><p className="mt-1 text-sm text-slate-400">{s.correct}/{s.total} correct · {s.attempted} attempted</p></div>)}
        </div>
        <div className="space-y-4">
          {questions.map((q, i) => <div key={q.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><b>{i+1}. {q.prompt}</b><p className="mt-2 text-sm text-slate-400">Your answer: {answers[q.id] || "Not answered"}</p><p className="text-sm text-emerald-300">Correct: {q.answer}</p><p className="mt-2 text-sm leading-6 text-slate-400">{q.explanation}</p>{q.sourceUrl && <a className="mt-2 inline-block text-xs font-bold text-cyan-300" href={q.sourceUrl} target="_blank" rel="noopener noreferrer">{q.source || "Source"} · {q.license || "source"} ↗</a>}</div>)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6 rounded-2xl border border-slate-800 bg-slate-900 p-4 text-sm text-slate-300">No login required · evaluation runs in your browser · +{marksPerCorrect} correct · −{negativeMarks} wrong · no AI request per attempt.</div>
      ...
      <button onClick={finish} className="mt-7 w-full rounded-xl bg-cyan-400 px-6 py-4 text-lg font-black text-slate-950">Finish & evaluate</button>
    </div>
  );
}
