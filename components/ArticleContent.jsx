...
function repairPdfBoldArtifacts(value = "") {
  return String(value).replace(/([\p{L}\p{N}])\*{4}(?=[\p{L}\p{N}])/gu, "$1").split("\n").map((line) => { const markers=line.match(/\*\*/g)||[]; return markers.length===1?line.replace(/\*\*/,""):line; }).join("\n");
}
...
function normalizeMarkdown(value = "") {
  ...
  return highlightMarkdownFacts(repairPdfBoldArtifacts(withoutHtml.replace(/^\s*\[\[CA_(?:START|END)\]\]\s*$/gim,"").replace(/^\s*CA_(?:TITLE|CATEGORY|GS|DATE|IMAGE)\s*:\s*.*$/gim,"").replace(/^\s*CurrentPulse AI\s*\|\s*STRICT CA UPLOAD FORMAT.*$/gim,"").replace(/^\s*(?:Page\s*)?\d+\s*(?:of\s*\d+)?\s*$/gim,"").replace(/\r\n?/g,"\n"))
