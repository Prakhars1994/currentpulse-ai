...
import { assessSourceEntailment } from "@/lib/editorial/sourceEntailmentGuard";
import { buildSourceGroundedNewsFallback } from "@/lib/ai/trustedCoverageFallback";
import { createDeadline } from "@/lib/network/deadline";
...
function enforceSourceEntailment(source, generated, label = "news") {
  const assessment = assessSourceEntailment(source, generated);
  if (!assessment.allowed) {
    throw new Error(`PUBLICATION_BLOCKED: ${assessment.code}: ${label}: ${assessment.reason}`);
  }
}
...
function newsCreatedAt(sourceItem = {}, generationMode = "news", fallback = new Date().toISOString()) {
  if (generationMode !== "news") return fallback;

  const raw = sourceItem.publishedAt || sourceItem.published_at || "";
  const parsed = raw ? new Date(raw) : null;
  if (!parsed || Number.isNaN(parsed.getTime())) return fallback;

  // Small publisher clock errors are tolerated; large future dates were
  // already rejected by the candidate safety gate.
  if (parsed.getTime() > Date.now() + 12 * 60 * 60 * 1000) return fallback;
...
export async function publishArticle(
  ...
  const newsNetworkDeadlineAt = sourceItem.trustedCoverage
    ? 0
    : createDeadline(
        Math.max(
          3000,
          Number(process.env.NEWS_ENRICHMENT_DEADLINE_MS) || 12000
        )
      );
  sourceItem = await enrichNewsSource(sourceItem, {
    deadlineAt: newsNetworkDeadlineAt,
  });
  ...
  const duplicateSnapshot = Array.isArray(sourceItem.duplicateSnapshot)
    ? sourceItem.duplicateSnapshot
    : null;
  const sourceDuplicate = duplicateSnapshot
    ? findDuplicateInArticles(
        {
          title: sourceItem.title,
          description: sourceItem.description || sourceItem.content || "",
  ...
  const sourceMaterial = createSourceMaterial(sourceItem);
  // General News is intentionally zero-AI. The deterministic source brief is
  // still checked by numeric, entailment and public-output guards below. This
  // keeps News fresh during provider outages and preserves scarce AI quota for
  // coaching Current Affairs where synthesis materially improves study value.
  const generatedArticle = generationMode === "news"
    ? buildSourceGroundedNewsFallback(sourceMaterial, {
        sourceTitle: sourceItem.title,
        sourceCategory: sourceItem.category,
        sourcePaper: sourceItem.paper,
      })
    : await generateArticle(sourceMaterial, {
        mode: generationMode,
        sourceTitle: sourceItem.title,
        sourceCategory: sourceItem.category,
        sourcePaper: sourceItem.paper,
        // Trusted coaching extracts may be converted into a source-grounded
        // emergency brief, but the publication quality gate below keeps weak
        // Current Affairs fallbacks pending until a strong pass is possible.
        allowTrustedFallback: Boolean(sourceItem.trustedCoverage),
        allowSourceFallback: false,
      });

  if (sourceItem.trustedCoverage && generatedArticle.__sourceFallback) {
    const fallbackQuality = Number(generatedArticle.quality?.score || 0);
    const fallbackFlags = Array.isArray(generatedArticle.quality?.flags)
      ? generatedArticle.quality.flags
      : [];
    if (
      fallbackQuality < 80 ||
      fallbackFlags.some((flag) =>
        ["thin_source_extract", "insufficient_source_evidence", "insufficient_source_depth"].includes(flag)
      )
    ) {
      throw new Error(
        "AI temporarily unavailable: retained coaching evidence is not rich enough for a public-quality fallback; the item will remain queued for a stronger AI pass."
      );
    }
  }
  ...
  if (
    ...
    Number(generatedArticle.quality?.score || 0) < 72
  ) {
    throw new Error(
      "PUBLICATION_BLOCKED: trusted Current Affairs article is below the strengthened public quality floor."
    );
  }

  enforceNumericGuard(sourceItem, generatedArticle, "new publication");
  if (generationMode === "news") enforceSourceEntailment(sourceItem, generatedArticle, "new publication");
  ...
  const duplicateEvent = duplicateSnapshot
    ? findDuplicateInArticles(
        {
  ...
  const imageDeadlineAt = newsNetworkDeadlineAt || createDeadline(12000);
  const governmentImage = await resolveGovernmentArticleImage({
    title: generatedArticle.title,
    category,
  }, { deadlineAt: imageDeadlineAt });
  const selectedImage = governmentImage.image?.url || "";
  ...
  const articleData = {
    ...
    image_caption: governmentImage.image?.attribution || null,
    image_source_url: governmentImage.image?.sourcePageUrl || null,
    image_resolution: governmentImage.resolution,
  ...
  const { data: publishedArticle, error: insertError } =
    await supabase
      .from("articles")
      .insert([articleData])
      .select("id,title,slug,category,paper,quality_score,why_news")
      .single();
...
export async function rebuildPublishedNewsArticle(supabase, articleId, sourceItem = {}) {
  ...
  if (fetchError) {
    throw new Error(`Existing News fetch failed: ${fetchError.message}`);
  }
  if (!existingArticle) {
    throw new Error("Published News article was not found for quality rebuild.");
  }

  const normalizedSource = {
    ...sourceItem,
    title:
      cleanText(sourceItem.title || sourceItem.source_title) ||
    ...
    publishedAt:
      ...
      sourceItem.source_published_at ||
      "",
    generationMode: "news",
  ...
  const enriched = await enrichNewsSource(normalizedSource, { deadlineAt });
  const sourceAssessment = assessNewsCandidate(enriched);

  if (!sourceAssessment.allowed) {
    throw new Error(
      `PUBLICATION_BLOCKED: ${sourceAssessment.code}: ${sourceAssessment.reason}`
    );
  }

  const newsVersion = buildSourceGroundedNewsFallback(
    createSourceMaterial(enriched),
    {
  ...
  enforceNumericGuard(enriched, newsVersion, "News quality rebuild");
  enforceSourceEntailment(enriched, newsVersion, "News quality rebuild");
  assertPublicOutput(newsVersion, "news");

  const content = serializeNewsPresentation(newsVersion);
  const outputQuality = assessNewsOutputQuality({
    ...newsVersion,
    content,
  });

  if (!outputQuality.allowed) {
    throw new Error(
      `PUBLICATION_BLOCKED: ${outputQuality.code}: ${outputQuality.reason}`
    );
  }
  ...
  const classificationText = [
    ...
    enriched.content,
    newsVersion.why_news,
    newsVersion.static_foundation,
    newsVersion.data_examples,
  ]
    .filter(Boolean)
    .join(" ");
  ...
  const category = classifyNewsCategory(
    classificationText,
    existingArticle.category || newsVersion.category
  );
  ...
  const { data: updatedArticle, error: updateError } = await supabase
    ...
    .update({
      ...
      paper: "Prelims",
      content,
      why_news: newsVersion.why_news,
      static_foundation: newsVersion.static_foundation,
      data_examples: newsVersion.data_examples,
      india_relevance: newsVersion.india_relevance,
      map_locations: newsVersion.map_locations,
      image_search_query: newsVersion.image_search_query,
      seo_description: stripHtml(newsVersion.why_news).slice(0, 155),
      quality_score: newsVersion.quality?.score || 82,
      quality_flags: newsVersion.quality?.flags || ["source_depth_verified"],
      quality_version: 4,
      updated_at: now,
    })
  ...
  if (updateError) {
    throw new Error(`News quality rebuild failed: ${updateError.message}`);
  }
...
export async function attachNewsPresentationToExistingArticle(supabase, articleId, sourceItem = {}) {
  ...
  const enrichedSourceItem = await enrichNewsSource(sourceItem, {
    deadlineAt: createDeadline(12000),
  });
  ...
  if (!snapshot) {
    const newsVersion = buildSourceGroundedNewsFallback(
      createSourceMaterial(enrichedSourceItem),
      {
    ...
    enforceNumericGuard(enrichedSourceItem, newsVersion, "news attachment");
    enforceSourceEntailment(enrichedSourceItem, newsVersion, "news attachment");
    assertPublicOutput(newsVersion, "news");
...
export async function enrichPublishedArticle(supabase, articleId, sourceItem) {
  ...
  const { category, paper } = taxonomy;
  const imageDeadlineAt = createDeadline(12000);
  const governmentImage = await resolveGovernmentArticleImage({
    ...existingArticle,
    title: existingArticle.title,
    category,
  }, { deadlineAt: imageDeadlineAt, preserveExisting: Boolean(currentImage) });
  const selectedImage = currentImage || governmentImage.image?.url || "";
  const resolvedImage = await persistRemoteArticleImage(
    supabase,
    selectedImage,
  ...
  const { data: updatedArticle, error: updateError } = await supabase
    ...
    .update({
      ...
      image_caption: currentImage ? existingArticle.image_caption : governmentImage.image?.attribution || null,
      image_source_url: currentImage ? existingArticle.image_source_url : governmentImage.image?.sourcePageUrl || null,
      image_resolution: governmentImage.resolution,
