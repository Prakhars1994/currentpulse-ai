...
function assertPublicOutput(article, stream) {
  ...
  if (stream === "coverage") {
    const eventness = assessCoverageEventness({
      ...article,
      summary: article.why_news || article.description || article.content || "",
    });
    if (!eventness.allowed) {
      throw new Error(`PUBLICATION_BLOCKED: ${eventness.code}: ${eventness.reason}`);
    }
  }
}
...
export async function publishArticle(
  ...
  // Check the retained source headline before spending AI quota. A second check
  // remains after generation because the editor may rewrite the headline.
  ...
  const generatedArticle = await generateArticle(createSourceMaterial(sourceItem), {
    ...
    // Trusted coaching extracts may publish as clearly marked, source-grounded
    // briefs when AI quota is unavailable. Set the variable to "false" only
    // for an emergency shutdown; otherwise useful daily CA must not stall.
    // Retained coaching extracts are already source-grounded. Never let a
    // stale deployment flag turn an AI formatting outage into a CA blackout.
...
export async function enrichPublishedArticle(supabase, articleId, sourceItem) {
  ...
  const enriched = await generateArticle(existingArticleSourceMaterial(existingArticle, sourceItem), {
