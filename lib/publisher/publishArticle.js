...
export async function publishArticle(
  ...
  const [generatedArticle, resolvedImage] = await Promise.all([
    ...
    suppliedImage
      ? Promise.resolve(suppliedImage)
      : extractImageFromArticle(
          cleanText(sourceItem.url),
          cleanText(sourceItem.source_domain || sourceItem.sourceDomain),
          cleanText(sourceItem.title)
        ),
  ]);
...
export async function enrichPublishedArticle(supabase, articleId, sourceItem) {
  ...
  const [enriched, resolvedImage] = await Promise.all([
    ...
    currentImage || suppliedImage
      ? Promise.resolve(currentImage || suppliedImage)
      : extractImageFromArticle(
          cleanText(sourceItem.url),
          cleanText(sourceItem.source_domain || sourceItem.sourceDomain),
          cleanText(sourceItem.title)
