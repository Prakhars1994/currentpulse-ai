...
export function cleanSeoTitle(value="",limit=68){const text=stripCaDecorators(value).replace(/\s+/g," ").trim();if(text.length<=limit)return text;return text.slice(0,limit).replace(/\s+\S*$/,"").replace(/[,:;\-–—]+$/,"").trim();}
export function cleanSeoDescription(value="",title="",limit=160){return cleanPublicExcerpt(value,title,limit).replace(/\s+/g," ").trim();}
export function selectSeoDescription(values=[],title="",limit=160){const candidates=values.map(value=>cleanSeoDescription(value,title,limit)).filter(Boolean);const useful=candidates.find(value=>value.length>=45&&!/^(?:category|gs|date|quick(?:\s+rule)?)(?:\s|$)/i.test(value));if(useful)return useful;const fallback=`${cleanSeoTitle(title,110)} — UPSC Current Affairs analysis with key facts and exam context.`;return cleanSeoTitle(fallback,limit);}
