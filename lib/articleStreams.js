...
const LIST_STREAM_FIELDS_INNER = `
  id,title,slug,category,paper,why_news,syllabus_linkage,india_relevance,
  static_foundation,data_examples,prelims,mains,answer_framework,question,
  visual_summary,memory_trick,content,seo_description,
...
async function fetchCurrentAffairsChunk(startIndex, endIndex, dayStart = "", dayEnd = "") {
  const key = `ca:${startIndex}:${endIndex}:${dayStart}:${dayEnd}`;
  return ttlCached(key, async () => {
    const supabase = createServerSupabase();
    let query = supabase
      .from("articles")
      .select(LIST_STREAM_FIELDS_INNER)
      .eq("status", "published")
...
export async function loadCurrentAffairsArticles({ limit = 24, offset = 0, todayOnly = false, exam = "upsc" } = {}) {
