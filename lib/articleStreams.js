...
/** Load the complete deduplicated CA archive once, then paginate the cleaned rows. */
export async function loadCurrentAffairsArticles({ limit = 48, offset = 0, todayOnly = false } = {}) {
  ...
  while (rawOffset < MAX_RAW_SCAN && !exhausted) {
  ...
  const prepared = dedupeStream(rawRows.filter(isCurrentAffairsReady));
  return {
