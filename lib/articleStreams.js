...
const FULL_STREAM_FIELDS = `
  ...
  article_sources(source_kind,source_name,source_title,source_url,source_published_at,source_key)
`;
...
export function isLegacyNewsArticle() {
  // Public News is deliberately PDF-only; historical unsourced rows are not
  // eligible for display.
  return false;
}

export function isCurrentAffairsReady(article = {}) {
  // The administrator's PDF is the sole editorial authority for public CA.
  return hasAdminPdfSource(article);
}

export function isPublicNewsArticle(article = {}) {
  // The administrator's PDF is the sole editorial authority for public News.
  return hasAdminPdfSource(article);
}
