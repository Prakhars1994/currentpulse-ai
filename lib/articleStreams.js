...
const CURRENT_AFFAIRS_LIST_FIELDS = `
  ...
  image,image_url,image_source_url,image_caption,image_search_query,created_at,views,status,quality_score,
  article_sources(source_kind,source_name,source_url,source_published_at)
`;
...
const NEWS_LIST_FIELDS_INNER = `
  id,title,slug,category,paper,why_news,syllabus_linkage,prelims,mains,content,
  image,image_url,image_source_url,image_caption,image_search_query,created_at,views,status,quality_score,
  article_sources!inner(source_kind,source_name,source_url,source_published_at)
`;
...
export function isCurrentAffairsReady(article = {}) {
  ...
  const sourceDates = coachingSources
  ...
  return assessCoverageEventness(
    {
      ...
      publishedAt: sourceDates[0] || article.created_at,
    },
...
const fetchCurrentAffairsChunk = unstable_cache(
  ...
  { revalidate: 60, tags: ["currentpulse-articles", "currentpulse-current-affairs"] }
);
...
const fetchNewsChunk = unstable_cache(
  ...
  { revalidate: 60, tags: ["currentpulse-articles", "currentpulse-news"] }
);
...
const fetchLegacyNewsFallback = unstable_cache(
  ...
  { revalidate: 300, tags: ["currentpulse-articles", "currentpulse-news"] }
);
...
export async function loadCurrentAffairsArticles({ limit = 48, offset = 0, todayOnly = false } = {}) {
  ...
  const RAW_BATCH_SIZE = 120;
  const MAX_STREAM_SCAN = Math.max(600, targetCount * 4);
