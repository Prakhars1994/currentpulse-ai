...
const GENERIC_EXAM_RESULT_TITLE = /^(?:preliminary|main|final|written)?\s*examination\s+result$/i;
...
export function resultPulseDisplayTitle(row = {}) {
  const title = examDisplayTitle(row.title) || clean(row.title);
  const examName = clean(row.exam_name);
  const authority = clean(row.source_name || row.agency);
  if (!title) return examName || authority || "Official exam update";
  const generic = GENERIC_EXAM_ACTION_TITLE.test(title) || GENERIC_EXAM_RESULT_TITLE.test(title);
  if (!generic) return title;
  const context = examName && !/^(?:nta|sbi careers|uppsc|rpsc|upsc)$/i.test(examName) ? examName : authority;
  return context && !title.toLowerCase().includes(context.toLowerCase()) ? `${title} — ${context}` : title;
}
...
export function assessExamSitemapRecord(row = {}) {
  ...
  if (GENERIC_EXAM_ACTION_TITLE.test(title) || GENERIC_EXAM_RESULT_TITLE.test(title) || /^(?:download|read more|notification|notice)$/i.test(title)) {
