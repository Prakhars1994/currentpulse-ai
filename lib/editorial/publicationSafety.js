...
const TRUSTED_COVERAGE_SOURCE_PATTERN = /\b(?:vision ias|drishti ias|insights ias|insightsias|forumias|next ias|vajiram|iasbaba|gktoday|gk today)\b/i;
...
function isTrustedCoverageCandidate(candidate = {}) {
  if (/trusted-coaching-coverage/i.test(clean(candidate.source_domain))) return true;
  return sourceEntries(candidate).some((source) =>
    TRUSTED_COVERAGE_SOURCE_PATTERN.test(source.name)
  );
}
