...
function isTrustedCoverageCandidate(candidate = {}) {
  return hasApprovedUpscCoverageSource(candidate);
}
...
const PUBLIC_EDITORIAL_RESIDUE_PATTERNS = [
...
export function findPublicEditorialResidue(value = "") {
  const text = String(value || "");
  const pattern = firstMatch(text, PUBLIC_EDITORIAL_RESIDUE_PATTERNS);
  return pattern ? pattern.source : "";
}
...
function dateAgeDays(value, referenceDate = null) {
  ...
  // News freshness is a calendar-day policy, not an exact 24-hour stopwatch.
  // A fixture/source dated seven days ago must not become stale merely because
  // today's job happens to run several hours later in the day.
  ...
  return (referenceDay - sourceDay) / 86400000;
}

/**
 * Trusted Current Affairs sources are already the editorial selector. Once an
 * item comes from an approved CA adapter, CurrentPulse must not second-guess it
 * with an importance/eventness threshold. We retain eventness only as
 * diagnostic metadata for non-trusted/manual coverage inputs. Structural
 * document/source-policy checks still block navigation, promotional and
 * non-article pages.
 */
export function assessCoverageEventness(candidate = {}, options = {}) {
  ...
  if (isTrustedCoverageCandidate(candidate)) {
    return {
      allowed: true,
      eventness: 10,
      code: "trusted_current_affairs_source",
      reason: "Accepted because the item came from a configured trusted Current Affairs source and passed structural sanitation.",
    };
  }
  ...
  // These adapters already target dated daily-current-affairs pages from
  // trusted coaching sources. Do not throw away a substantive daily brief just
  // because its heading is noun-like (for example, a place/species/report in
  // news or an editorial headline without a verb). Strict document, URL and
  // promotional-page rejection has already run above. Old dated pages still
  // require an explicit current trigger.
  const trustedCoverage = isTrustedCoverageCandidate(candidate);
  const trustedDailyBrief =
    trustedCoverage &&
    summary.length >= 120 &&
    (recentSource || ageDays === null);

  if (trustedDailyBrief) eventness = Math.max(eventness, 6);
  ...
  return {
    ...
    reason: allowed
      ? trustedDailyBrief
        ? "A substantive brief from a configured trusted daily-current-affairs source passed document and source sanitation."
...
export function assessNewsCandidate(candidate = {}) {
  ...
  // Normal incoming News is checked against today. Legacy queued News may carry
  // a freshnessReferenceDate equal to the time it originally entered the queue,
  // so a processing delay cannot turn a once-fresh story into a false rejection.
  const ageDays = dateAgeDays(
    publishedAt,
    candidate.freshnessReferenceDate || null
  );
...
export function assessPublishedArticle(article = {}, { stream = "news" } = {}) {
  ...
  const leak = findInternalPipelineLeak(publicText);
  if (leak) {
  ...
  const residue = findPublicEditorialResidue(publicText);
  if (residue) {
    return { allowed: false, code: "ai_editorial_residue", reason: "Raw AI key-value or extraction residue appeared in public article fields." };
  }
