...
function normalize(value = "") { return clean(value).toLowerCase().replace(/[’']/g, "").replace(/[^a-z0-9%]+/g, " ").replace(/\s+/g, " ").trim(); }
function sourceText(source = {}) { return clean([source.title,source.description,source.content,...(Array.isArray(source.sourceReferences)?source.sourceReferences.flatMap((item)=>[item?.sourceTitle,item?.summary,item?.content]):[])].filter(Boolean).join("\n")); }
function generatedText(article = {}) { return clean([article.title,article.why_news,article.data_examples,article.static_foundation,article.india_relevance,article.content].filter(Boolean).join("\n")); }
...
function entities(value = "") {
  const text=clean(value), found=new Set();
...
export function assessSourceEntailment(source = {}, article = {}) {
  ...
  const unsupportedEntities=entities(generatedValue).filter((entity)=>!supportedPhrase(entity,sourceNormalized)).slice(0,12);
