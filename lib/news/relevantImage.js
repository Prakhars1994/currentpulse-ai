...
const STOP_WORDS = new Set([
  ...
  "under", "with", "will", "india", "indian", "latest", "news",
]);
...
function candidateScore(page, query) {
  ...
  if (
    ...
    !candidateYears.some((year) => queryYears.includes(year))
  ) {
    return -1000;
  }
  const minimumMatches = Math.min(2, queryTokens.length);
  if (matches < minimumMatches && ratio < 0.55) return -1000;
  const landscapeBonus = Number(info.width) / Number(info.height) >= 1.25 ? 1.5 : 0;
  const photoBonus = String(info.mime).includes("jpeg") ? 1 : 0;
  const genericPenalty = /\b(icon|logo|coat of arms|blank map|locator map|symbol)\b/i.test(haystack)
    ? 3
    : 0;
