...
async function searchCommons(query, deadlineAt = 0) {
  ...
  const signal = deadlineSignal(deadlineAt, 18000);
  if (!signal) return [];
...
export async function findRelevantCommonsImage(imageQuery, title, options = {}) {
  const queries = createSearchQueries(imageQuery, title);
  let best = null;

  for (const query of queries) {
    if (!remainingDeadlineMs(options.deadlineAt, 18000)) break;
    try {
      const pages = await searchCommons(query, options.deadlineAt);
      const ranked = pages
