...
export function isSafeDestructiveDuplicate(articleA = {}, articleB = {}) {
  if (!isSameEvent(articleA, articleB)) return false;
  const first = generateEventFingerprint(articleA);
  const second = generateEventFingerprint(articleB);
  if (
    first.normalizedTitle &&
  ...
  const signature = overlap(first.signature, second.signature);
  const eventKeyMatch =
    first.eventKey && second.eventKey && first.eventKey === second.eventKey;

  // Destructive maintenance is deliberately stricter than display/queue
  // grouping. Preserve both articles unless the rewritten headlines share a
  // strong core or a distinctive multi-part signature.
  return Boolean(
    (signature.common >= 2 && title.common >= 3) ||
      (phrases.common >= 1 && title.common >= 2 && title.containment === 1) ||
      (title.common >= 4 && title.containment >= 0.78 && title.jaccard >= 0.48) ||
      (title.common >= 3 && title.containment >= 0.9 && title.jaccard >= 0.55) ||
      (eventKeyMatch && title.common >= 4 && title.containment >= 0.72)
  );
}
