...
import { deadlineSignal } from "@/lib/network/deadline";
...
async function fetchArticleHtml(inputUrl, deadlineAt = 0) {
  let current = safePublicUrl(inputUrl);
  if (!current) return null;

  for (let redirects = 0; redirects <= 3; redirects += 1) {
    const signal = deadlineSignal(deadlineAt, 15000);
...
export async function enrichNewsSource(sourceItem = {}, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (parsed.hostname === "news.google.com" || parsed.hostname.endsWith(".news.google.com")) {
        articleUrl = await resolveGoogleNewsPublisherUrl(
          articleUrl,
          sourceItem.sourceDomain || "",
          options
        );
    ...
    const fetched = await fetchArticleHtml(articleUrl, options.deadlineAt);
