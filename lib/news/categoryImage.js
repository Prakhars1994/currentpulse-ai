...
// Prefer a verified article-specific image. If enrichment has not happened yet,
// render a deterministic zero-API category image so live cards never look broken.
// The publication resolver still remains Wikimedia-first -> official-source fallback.
export function resolveCardImage(article={}){const storedImage=article.image||article.image_url||"";return isVerifiedReusableArticleImage(article)?storedImage:getCategoryFallbackImage(article.category,article.slug||article.title);}
