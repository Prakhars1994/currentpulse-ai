...
const GOVERNMENT_IMAGE_PROVIDERS = new Set([
  "isro", "nasa", "noaa", "usgs", "pib", "usda_ars", "nps", "cdc_nih",
]);
...
export function isVerifiedReusableArticleImage(article = {}) {
  ...
  const resolution = article.image_resolution || {};
  const governmentResolved =
    resolution.status === "resolved" &&
    GOVERNMENT_IMAGE_PROVIDERS.has(resolution.provider) &&
    Boolean(article.image_source_url) &&
    (imageHost.includes("supabase.co") || sourceHost);
  const sourcedGovernmentImage =
    Boolean(article.image_caption && article.image_source_url) &&
    ["nasa.gov", "isro.gov.in", "noaa.gov", "usgs.gov", "pib.gov.in", "usda.gov", "nps.gov", "cdc.gov", "nih.gov"]
      .some((domain) => sourceHost === domain || sourceHost.endsWith(`.${domain}`));
  const preservedExisting =
    resolution.status === "preserved_existing" &&
    Boolean(article.image_source_url || article.image_caption);
  const reusableHost = (
  ...
  return governmentResolved || sourcedGovernmentImage || preservedExisting ||
    (reusableHost && isStoredImageSemanticallyRelevant(article));
}
