...
export function isVerifiedReusableArticleImage(article={}){const storedImage=article.image||article.image_url||"";if(!storedImage||isGenericFallbackImage(storedImage))return false;if(String(storedImage).startsWith("/api/article-visual"))return false;const imageHost=hostname(storedImage);const sourceHost=hostname(article.image_source_url||"");const resolution=article.image_resolution||{};const governmentResolved=resolution.status==="resolved"&&GOVERNMENT_IMAGE_PROVIDERS.has(resolution.provider)&&Boolean(article.image_source_url)&&(imageHost.includes("supabase.co")||sourceHost);const sourcedGovernmentImage=Boolean(article.image_caption&&article.image_source_url)&&["nasa.gov","isro.gov.in","noaa.gov","usgs.gov","pib.gov.in","usda.gov","nps.gov","cdc.gov","nih.gov"].some((domain)=>sourceHost===domain||sourceHost.endsWith(`.${domain}`));const preservedExisting=resolution.status==="preserved_existing"&&Boolean(article.image_source_url||article.image_caption);const reusableHost=imageHost==="upload.wikimedia.org"||imageHost==="commons.wikimedia.org"||sourceHost==="commons.wikimedia.org"||sourceHost.endsWith(".commons.wikimedia.org");const wikimediaResolved=resolution.status==="resolved"&&resolution.provider==="wikimedia"&&reusableHost&&Boolean(article.image_caption&&article.image_source_url);const openverseResolved=isSafeOpenverseResolution(resolution)&&imageHost==="api.openverse.org"&&Boolean(article.image_caption&&article.image_source_url);const unsplashImage=imageHost==="images.unsplash.com"&&sourceHost.includes("unsplash.com")&&Boolean(article.image_caption);return governmentResolved||sourcedGovernmentImage||preservedExisting||unsplashImage||wikimediaResolved||openverseResolved||(reusableHost&&isStoredImageSemanticallyRelevant(article));}
function isDirectRenderableImage(value=""){const host=hostname(value);return host==="upload.wikimedia.org"||host==="images.unsplash.com"||host==="api.openverse.org"||host.includes("supabase.co")||/\.(?:jpg|jpeg|png|webp|gif)(?:\?|$)/i.test(String(value||""));}
// Archive cards must show a real, reusable, sourced image or no image at all.
// Generated blue placeholder visuals are intentionally excluded here.
...
// Detail pages may still opt into a generated visual explicitly if desired.
