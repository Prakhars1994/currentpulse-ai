...
function isSourceGroundedNewsFallback(article = {}) {
  const runtimeFlags = Array.isArray(article.quality?.flags)
    ? article.quality.flags
    : [];
  const persistedFlags = Array.isArray(article.quality_flags)
    ? article.quality_flags
    : [];

  return (
    article.__sourceFallback === true ||
    runtimeFlags.includes("source_grounded_news_fallback") ||
    persistedFlags.includes("source_grounded_news_fallback")
  );
}
...
export function assessNewsOutputQuality(article = {}) {
  ...
  if (presentation) {
    ...
    if (byName.lead.length < 100) {
      return { allowed: false, code: "thin_news_lead", reason: "News lead is too thin." };
    }
    if (byName.keyFacts.length < 90) {
      return { allowed: false, code: "thin_news_facts", reason: "News key facts are too thin." };
    }
    if (byName.context.length < 90) {
      return { allowed: false, code: "thin_news_context", reason: "News context is too thin." };
    }
    const coreDepth = clean(`${byName.lead} ${byName.keyFacts} ${byName.context}`).length;
    if (coreDepth < 320) {
      return {
        allowed: false,
        code: "insufficient_news_depth",
        reason: "News does not contain enough source-backed explanation.",
      };
    }
  }
