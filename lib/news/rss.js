...
import { historyDateWindow } from "@/lib/automation/history";
...
export async function fetchWithTimeout(
  ...
  timeout = Math.max(
    4_000,
    Number(process.env.NEWS_FETCH_TIMEOUT_MS) || 8_000
  )
) {
  ...
  try {
    ...
    // Keep the abort timer alive while consuming the body. Headers arriving
    // before the deadline must not allow a stalled RSS body to run forever.
