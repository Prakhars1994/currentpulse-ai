...
/**
 * Stable visibility gate for News that is already published.
 *
 * Publication-time depth/format requirements must not be retroactively
 * applied to the historical archive. A previously valid story remains
 * visible unless it violates durable public-safety, junk or date-integrity
 * rules.
 */
export function isArchiveWorthyNews(article = {}) {
...
export function isDisplayWorthyNews(article = {}) {
  if (!isArchiveWorthyNews(article)) return false;
