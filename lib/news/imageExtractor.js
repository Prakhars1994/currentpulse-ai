...
import { remainingDeadlineMs } from "@/lib/network/deadline";
...
async function fetchHtml(url, deadlineAt = 0) {
  const controller = new AbortController();
  const timeoutMs = remainingDeadlineMs(deadlineAt, FETCH_TIMEOUT_MS);
  if (!timeoutMs) {
    return { html: "", finalUrl: "", status: 0, ok: false };
  }
...
export async function resolveGoogleNewsPublisherUrl(
  ...
  sourceDomain || "not provided"
    ...
    for (const urlVariant of googleUrlVariants) {
      if (!remainingDeadlineMs(options.deadlineAt, FETCH_TIMEOUT_MS)) break;
      const { html, finalUrl } =
        await fetchHtml(urlVariant, options.deadlineAt);
