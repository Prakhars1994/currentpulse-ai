...
/**
 * Conservative deterministic screen. It only hard-rejects obvious routine
 * foreign-country material; borderline items still go to the AI editor.
 */
export function assessUpscRelevance(article = {}) {
  ...
  const domesticSource = region === "in" || sourceGroup === "indian-news";
  ...
  if (india.length || domesticSource) {
    return {
      ...
      hardReject: false,
      scope: "India",
      scoreAdjustment: india.length ? 3 : 1,
      reason: india.length
