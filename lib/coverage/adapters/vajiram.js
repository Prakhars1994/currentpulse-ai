...
function buildRecentUrls(days = 7, historyDate = "") {
  const history = historyDateParts(historyDate);
  if (history) {
    const base = `${BASE_URL}/current-affairs`;
    const suffix = `${history.year}/${history.month}/${history.day}/`;
    return [
      `${base}/upsc-prelims-current-affairs/${suffix}`,
      `${base}/upsc-mains-current-affairs/${suffix}`,
    ];
  }
  const urls = [];
  const now = new Date();
...
export async function fetchVajiramTopics({ historyDate = "" } = {}) {
  ...
  const urls = buildRecentUrls(7, historyDate);
