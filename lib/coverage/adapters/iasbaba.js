...
export async function fetchIasBabaTopics({ historyDate = "" } = {}) {
  const history = historyDateParts(historyDate);
  const links = history
    ? [{
        url: `${BASE_URL}/${history.year}/${history.month}/daily-current-affairs-ias-upsc-prelims-and-mains-exam-${history.ordinalDay}-${history.monthName.toLowerCase()}-${history.year}/`,
      }]
    : collectRecentDigestLinks(await fetchHtml(LIST_URL));
  if (!links.length) {
