...
function collectTopics($, pageUrl) {
  const topics = [];
  $("h4, h5, h6").each((_, heading) => {
    ...
    const fragment = title.toLowerCase().replace(/&/g, " and ")
...
export async function fetchVisionTopics({ historyDate = "" } = {}) {
  ...
  const pageUrl = date ? `${LIST_URL}/${date}/all` : LIST_URL;
  const html = await fetchHtml(pageUrl, 40000);
  const $ = loadHtml(html);
  return collectTopics($, pageUrl);
}
