...
export async function fetchForumTopics({ historyDate = "" } = {}) {
  const history = historyDateParts(historyDate);
  const links = history
    ? [{
        url: `${BASE_URL}/blog/9-pm-upsc-current-affairs-articles-${history.dayNumber}-${history.monthName.toLowerCase()}-${history.year}/`,
      }]
    : recentDigestLinks(await fetchHtml(LIST_URL));
