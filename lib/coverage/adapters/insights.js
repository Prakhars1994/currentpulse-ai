...
function buildRecentUrls(days = 7, historyDate = "") {
  ...
  if (history) {
    return [
      `${BASE_URL}/${history.year}/${history.month}/${history.day}/upsc-current-affairs-${history.dayNumber}-${history.monthName.toLowerCase()}-${history.year}/`,
    ];
  }
  const urls = [];
  const now = new Date();
...
export async function fetchInsightsTopics({ historyDate = "" } = {}) {
  const pageUrls = buildRecentUrls(7, historyDate);
