...
export function hasApprovedUpscCoverageSource(candidate = {}) {
  const entries = sourceEntries(candidate);

  // Internal authenticated CA-PDF import. This is not an eighth external
  // coaching source and does not change the approved coaching-source list.
  const internalAdminPdf = entries.some((entry) => {
    const name = normalizeName(entry.name);
    const host = hostname(entry.url);
    return (
      name === "currentpulse admin ca pdf" &&
      host === "cp.vliab.workers.dev"
    );
  });

  if (internalAdminPdf) return true;
