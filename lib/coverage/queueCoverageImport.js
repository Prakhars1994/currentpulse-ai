...
import { assessCoverageEventness } from "@/lib/editorial/publicationSafety";
...
const QUEUE_WRITE_CONCURRENCY = 6;
...
async function loadCoverageState(supabase, recentArticles) {
  const [sourceResult, queueResult] = await Promise.all([
    ...
    supabase
      .from("article_queue")
      .select(
        "id,status,attempts,coverage_event_key,coverage_sources,target_article_id"
      )
...
async function persistCoverageCandidate(supabase, candidate, queueByEvent) {
  const existing = queueByEvent.get(candidate.topic.eventKey);
...
export async function queueCoverageImport({
  ...
  const prefilteredTopics = [];
  const results = [];

  for (const sourceTopic of sourceTopics) {
    if (isCoverageNoiseTitle(sourceTopic.title)) {
      results.push({
    ...
    const eventness = assessCoverageEventness(sourceTopic);
    if (!eventness.allowed) {
      results.push({
        status: "noise_rejected",
        title: sourceTopic.title,
        reason: eventness.reason,
        eventness: eventness.eventness,
      });
      continue;
    }

    const taxonomy = correctTaxonomy(
      `${sourceTopic.title || ""} ${sourceTopic.summary || ""}`,
      sourceTopic.category,
      sourceTopic.paper
    );
    prefilteredTopics.push({
      ...sourceTopic,
  ...
  const topics = mergeCoverageTopics(prefilteredTopics);
  const state = await loadCoverageState(supabase, recentArticles);
  const candidates = [];

  for (const originalTopic of topics.slice(0, maxCandidates)) {
  ...
  const queuedResults = await mapWithConcurrency(
    ...
    async (candidate) => {
      try {
        return await persistCoverageCandidate(
          supabase,
          candidate,
          state.queueByEvent
        );
        ...
        console.error(
          `[Coverage queue] Failed for "${candidate.topic.title}":`,
          error?.message || error
        );
        return {
          status: "failed",
          title: candidate.topic.title,
          error: error?.message || "Coverage queue write failed.",
        };
      }
    }
  );
  ...
  return {
    ...
    considered: Math.min(topics.length, maxCandidates),
    ...
    eventnessAccepted: prefilteredTopics.length,
