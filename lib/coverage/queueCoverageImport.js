...
import {
  ...
  preferRicherCoverageReference,
} from "@/lib/coverage/evidence";
...
const COVERAGE_PERSIST_CONCURRENCY = 6;
...
const COVERAGE_SOURCE_TIMEOUT_MS = boundedInteger(
  process.env.COVERAGE_SOURCE_TIMEOUT_MS,
  45000,
  15000,
  60000
);
...
const configuredCoverageSourceIds = Object.keys(SOURCE_ADAPTERS);
const missingCoverageAdapters = COVERAGE_SOURCE_IDS.filter(
  (sourceId) => !SOURCE_ADAPTERS[sourceId]
);
const unexpectedCoverageAdapters = configuredCoverageSourceIds.filter(
  (sourceId) => !COVERAGE_SOURCE_IDS.includes(sourceId)
);

if (missingCoverageAdapters.length || unexpectedCoverageAdapters.length) {
  throw new Error(
    "Coverage source-policy drift detected. " +
      "Missing adapters: " +
      (missingCoverageAdapters.join(", ") || "none") +
      "; unexpected adapters: " +
      (unexpectedCoverageAdapters.join(", ") || "none") +
      "."
  );
}
...
async function withSourceDeadline(promise, sourceId) {
  let timer = null;
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(
          () => reject(
            new Error(
              `Coverage source ${sourceId} exceeded the ${COVERAGE_SOURCE_TIMEOUT_MS}ms source budget.`
            )
          ),
          COVERAGE_SOURCE_TIMEOUT_MS
        );
      }),
    ]);
  } finally {
...
async function fetchCoverageSources(requestedSourceIds, options = {}) {
  ...
  const settled = await Promise.allSettled(
    entries.map(([sourceId, fetchTopics]) =>
      withSourceDeadline(fetchTopics(options), sourceId)
    )
  );
  const groups = [];
  const counts = {};
...
function mergeReferences(...groups) {
  const byKey = new Map();
  for (const input of groups.flat().filter(Boolean)) {
    const reference = createCoverageSourceReference(input);
    if (!reference.sourceUrl) continue;
    const current = byKey.get(reference.sourceKey);
    byKey.set(
      reference.sourceKey,
      current ? preferRicherCoverageReference(current, reference) : reference
    );
  }
  return [...byKey.values()];
}

function queuedTopicPayload(topic, references, duplicate, evidence = null) {
  const prepared = topicWithCoverageSources(topic, references);
  const evidenceReady = evidence?.accepted !== false;
  ...
  return {
    ...
    status: evidenceReady ? "pending" : "rejected",
    ...
    error: evidenceReady ? null : evidence.reason,
    processing_started_at: null,
    processed_at: evidenceReady ? null : now,
    updated_at: now,
  };
}

async function loadCoverageState(supabase, recentArticles) {
  const sourceCutoff = new Date(Date.now() - 45 * 86_400_000).toISOString();
  const queueCutoff = new Date(Date.now() - 45 * 86_400_000).toISOString();

  const [sourceResult, queueResult] = await Promise.all([
    supabase
      .from("article_sources")
      .select("article_id,source_key")
      .gte("created_at", sourceCutoff)
...
async function persistCoverageCandidate(supabase, candidate, state) {
  ...
  const evidence = assessCoverageEvidence(references);
  ...
  if (existing) {
    ...
    const hasImprovedSource = references.some((reference) => {
      const previous = existingReferences.find(
        (item) => item?.sourceKey === reference.sourceKey
      );
      return Boolean(
        previous &&
          previous.contentHash !== reference.contentHash &&
          String(reference.summary || "").length >
            String(previous.summary || "").length
      );
    });
    ...
    return {
      status: payload.status === "rejected" ? "evidence_rejected" : "queue_updated",
      title: candidate.topic.title,
      queueId: existing.id,
  ...
  return {
    status: payload.status === "rejected" ? "evidence_rejected" : "queued",
    title: candidate.topic.title,
    queueId: data.id,
...
export async function queueCoverageImport({
  ...
  const [coverage, recentArticles] = await Promise.all([
    fetchCoverageSources(selectedSourceIds, { historyDate }),
  ...
  for (const originalTopic of candidateTopics) {
    ...
    candidates.push({
      ...
      references,
      duplicate,
      evidence: assessCoverageEvidence(references),
    });
  }

  const processedResults = await mapWithConcurrency(
    candidates,
    COVERAGE_PERSIST_CONCURRENCY,
    async (candidate) => {
      // Collection is intentionally AI-free. This endpoint only fetches,
      // sanitizes, deduplicates and persists durable queue rows. The bounded
      // queue processor owns all AI generation/publication work.
      ...
      try {
        ...
        return {
          ...
          ...(!shouldPublishImmediately
            ? {
                deferredReason: candidate.evidence.accepted
                  ? "Collection-only architecture: AI publication is handled by the bounded queue processor."
                  : candidate.evidence.reason,
                evidence: candidate.evidence.metrics,
              }
            : {}),
        };
      } catch (error) {
  ...
  return {
    ...
    evidenceRejected: countStatus("evidence_rejected"),
