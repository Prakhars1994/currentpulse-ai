...
async function fetchCoverageSources(requestedSourceIds) {
  ...
  const settled = await Promise.allSettled(
    entries.map(([sourceId, fetchTopics]) => withSourceDeadline(fetchTopics(), sourceId))
  );
  const groups = [];
  const counts = {};
  const errors = {};
      ...
      try {
        ...
        return {
          ...
          ...(!shouldPublishImmediately
            ? {
                deferredReason: "Collection-only architecture: AI publication is handled by the bounded queue processor.",
              }
            : {}),
        };
      } catch (error) {
