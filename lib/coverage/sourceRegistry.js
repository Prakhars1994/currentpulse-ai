...
export function createCoverageEventKey(topic = {}) {
  ...
  const fingerprint = generateEventKey({
    title: topic.title,
  ...
  return hash(`${cycleDate}|${fingerprint || normalizedTitle}`).slice(0, 32);
}
...
export async function recordArticleSources(supabase, articleId, topic) {
  ...
  const { error } = await supabase
    .from("article_sources")
    // A later crawl may retain a materially richer extract for the same
    // canonical source URL/title. Updating the registry lets that source repair
    // an earlier weak or wrongly-associated row instead of freezing the first
    // association forever.
