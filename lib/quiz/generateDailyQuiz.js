...
export async function generateDailyQuiz(supabase, { force = false } = {}) {
  ...
  let sourceResult = await loadCurrentAffairsArticles({ limit: 24, offset: 0, todayOnly: true });
  ...
  if (sourceResult.articles.length < 6) {
    sourceResult = await loadCurrentAffairsArticles({ limit: 24, offset: 0, todayOnly: false });
  }
