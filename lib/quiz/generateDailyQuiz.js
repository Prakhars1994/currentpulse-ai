...
import { loadCurrentAffairsDatePage, loadCurrentAffairsDates } from "@/lib/articleStreams";
...
function isGroundedInSource(prompt, explanation, source) {
  const sourceTokens = new Set(
    groundingTokens(
      `${source.title} ${source.why_news} ${source.syllabus_linkage} ` +
      `${source.india_relevance} ${source.static_foundation} ${source.data_examples} ` +
      `${source.prelims} ${source.mains}`
    )
  );
...
function validateQuestions(rawQuestions, articles, quizDate, provider) {
  ...
  for (const raw of rawQuestions) {
    ...
    validated.push({
      ...
      generation_provider: `${provider}|upsc-v2`,
...
function sourcePayload(articles) {
  return articles.slice(0, 8).map((article) => ({
    ...
    category: cleanText(article.category, 80),
    whyInNews: cleanText(article.why_news, 350),
    syllabusLinkage: cleanText(article.syllabus_linkage, 180),
    indiaRelevance: cleanText(article.india_relevance, 250),
    staticFoundation: cleanText(article.static_foundation, 450),
    dataReportsCasesExamples: cleanText(article.data_examples, 450),
    prelimsFacts: cleanText(article.prelims, 550),
    mainsContext: cleanText(article.mains, 220),
  }));
}
...
1. Questions must resemble the reasoning level and language of official UPSC Civil Services Prelims GS-I: test current-static linkage, conceptual distinctions, institutions, provisions, geography, ecology, economy, science mechanisms and consequences. Never ask the article title, website category, GS-paper mapping, publisher or headline.
...
4. Use only facts present in the corresponding source article, including its verified static foundation and evidence box. Do not introduce names, dates, statistics, provisions or claims absent from that article.
...
9. Avoid trivia that tests an isolated number or name with no conceptual value. Prefer combinations where a prepared candidate must distinguish scope, function, legal status, location, mechanism or implication.
10. At least four questions must link the current development to a stable syllabus concept; at least two must use a map/location, institution, report, Act, convention, species or scientific mechanism when the sources support it.
...
export async function generateDailyQuiz(supabase, { force = false } = {}) {
  const availableDates = await loadCurrentAffairsDates({ limit: 1 });
  const quizDate = availableDates.dates?.[0] || indiaDate();
  ...
  let sourceResult = await loadCurrentAffairsDatePage({ date: quizDate, limit: 24, offset: 0 });
  if (sourceResult.error) throw new Error(`Quiz source fetch failed: ${sourceResult.error.message}`);
  if (sourceResult.articles.length < 6) {
    sourceResult = await loadCurrentAffairsDatePage({ date: quizDate, limit: 48, offset: 0 });
  }
  const usable = (sourceResult.articles || []).filter(
    (article) =>
      article.title &&
      article.slug &&
      isPublishedArticleSafe(article, { stream: "coverage" }) &&
      Number(article.quality_version || 0) >= 2 &&
      Number(article.quality_score || 0) >= 76 &&
      article.static_foundation &&
      article.prelims
  );
  if (usable.length < 6) {
    throw new Error(
      "At least six quality-approved UPSC articles are required. The site will retain its curated foundation quiz instead of publishing weak questions."
    );
  }

  const generated = await requestQuiz(usable.slice(0, 8), quizDate);
