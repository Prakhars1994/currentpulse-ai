...
function isBoldFont(item = {}) {
  return /bold|black|heavy|semibold|demi/i.test(String(item.fontName || ""));
}
...
function joinLineItems(items = []) {
  ...
  for (const item of ordered) {
    ...
    const bold = isBoldFont(item);
    const previous = runs[runs.length - 1];
    if (previous && previous.bold === bold) {
      previous.text = previous.text.endsWith("-") && /^[a-z]/.test(value)
        ? previous.text.slice(0, -1) + value
        : `${previous.text} ${value}`;
    } else {
  ...
  const hasMixedWeight = runs.some((run) => run.bold) && runs.some((run) => !run.bold);
  const text = runs.map((run) =>
    hasMixedWeight && run.bold ? `**${run.text}**` : run.text
  ).join(" ");
...
function groupItemsIntoLines(items = []) {
  ...
  return buckets
    ...
    .map((bucket) => {
      ...
      const boldVotes = bucket.items.filter(isBoldFont).length;
