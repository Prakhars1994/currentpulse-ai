...
const SECTION_RULES = [
  ...
  ["question", /^(?:practice\s+question|mains\s+question|question|possible\s+question)\s*:?\s*$/i],
...
export function buildPdfImportPreview({
  ...
  if (!["ca", "news"].includes(stream)) {
    throw new Error("PDF import stream must be ca or news.");
  }
