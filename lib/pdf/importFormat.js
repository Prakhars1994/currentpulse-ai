...
function parseStrictMarkerDrafts(rows, stream) {
  ...
  if (stream !== "ca" && stream !== "ca_hi") {
    throw new Error("Invalid CurrentPulse CA PDF format. No safe article boundaries detected.");
  }

  const starts = [...text.matchAll(/\[\[CA_START\]\]/gi)];
  const ends = [...text.matchAll(/\[\[CA_END\]\]/gi)];
  if (!starts.length || starts.length !== ends.length || starts.some((s, i) => s.index >= ends[i].index || (starts[i + 1] && starts[i + 1].index < ends[i].index))) {
    throw new Error("Invalid CurrentPulse CA PDF format. No safe article boundaries detected.");
  }

  const drafts = [];
  for (let i = 0; i < starts.length; i += 1) {
    const block = text.slice(starts[i].index + starts[i][0].length, ends[i].index);
    const lines = block.split(/\r?\n/);
    const metadata = {};
    let bodyStart = 0;
    for (; bodyStart < lines.length; bodyStart += 1) {
      const line = lines[bodyStart].trim();
      const match = line.match(/^CA_(TITLE|CATEGORY|GS|DATE|IMAGE)\s*:\s*(.*)$/i);
      if (!match) {
        if (!line) continue;
        break;
      }
    ...
    if (!metadata.title) throw new Error("Invalid CurrentPulse CA PDF format. No safe article boundaries detected.");
    const body = lines.slice(bodyStart).join("\n").trim();
    drafts.push(buildDraft({ title: metadata.title, bodyLines: body ? body.split(/\r?\n/) : [] }, stream, i));
    ...
    draft.seo_description = clean(body).slice(0, 160);
...
export function buildPdfImportPreview({
  ...
  if (strictDrafts) {
    return {
      stream, fileName, fileHash, bodyFont: 0, drafts: strictDrafts,
      stats: { pages: pages.length, articlesDetected: strictDrafts.length,
        extractedChars: rows.reduce((sum, line) => sum + line.text.length, 0),
        preservedChars: strictDrafts.reduce((sum, article) => sum + article.fullText.length + article.title.length, 0),
        removedBoilerplate, zeroAi: true },
    };
  }
