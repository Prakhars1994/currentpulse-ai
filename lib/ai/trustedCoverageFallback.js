...
export function buildTrustedCoverageFallback(sourceContent, options = {}) {
  ...
  const sourceKnowledge = extractKnowledge(sourceContent);
  const sourceWordCount = sourceKnowledge.split(/\s+/).filter(Boolean).length;
  const fallbackFlags = ["source_grounded_fallback"];
  if (points.length < 12) fallbackFlags.push("thin_source_extract");
  if (evidence.length < 5) fallbackFlags.push("insufficient_source_evidence");
  if (sourceWordCount < 450) fallbackFlags.push("insufficient_source_depth");
  const fallbackQualityScore = Math.max(
    45,
    Math.min(84, 84 - Math.max(0, fallbackFlags.length - 1) * 12)
  );
  ...
  return {
    ...
    quality: {
      ...
      metrics: {
        ...
        sourceEvidenceItems: evidence.length,
...
export function buildSourceGroundedNewsFallback(sourceContent, options = {}) {
  const title = clean(options.sourceTitle) || "CurrentPulse News Brief";
  const category = normalizeCategory(options.sourceCategory);
  const paper = normalizePaper(options.sourcePaper);
  const points = sentencePoints(sourceContent);
  const sourceKnowledge = extractKnowledge(sourceContent);
  ...
  // A headline or short RSS snippet is not enough to publish a reader-facing
  // article. Full News requires enough source evidence for distinct sections.
  if (points.length < 7 || sourceWordCount < 120) {
    throw new Error(
      "PUBLICATION_BLOCKED: insufficient_news_depth: Full source evidence is too thin for a useful News article."
    );
  }

  const leadPoints = points.slice(0, 2);
  const factPoints = points.slice(2, 5);
  const contextPoints = points.slice(5, 7);
  const used = new Set([...leadPoints, ...factPoints, ...contextPoints]);

  const impactPattern =
    /\\b(?:impact|affect|means|significance|risk|benefit|cost|pressure|concern|challenge|consequence|because|therefore|could|may|will|expected|likely|important|matters)\\b/i;

  const impactPoints = points
    .filter((point) => !used.has(point) && impactPattern.test(point))
    .slice(0, 2);

  const mapLocations = verifiedMapLocations(`${title}\\n${sourceKnowledge}`);

  return {
    title,
    category,
    paper,
    why_news: leadPoints.map(emphasize).join(" "),
    syllabus_linkage: "",
    india_relevance: impactPoints.map(emphasize).join(" "),
    static_foundation:
      `### Essential context\\n${contextPoints.map(emphasize).join("\\n\\n")}`,
    data_examples:
    ...
    answer_framework: "",
    question: "",
    image_search_query: `${title} exact event place institution Wikimedia Commons`,
    visual_summary: "",
    memory_trick: "",
    map_locations: mapLocations,
    quality: {
      score: 82,
      flags: ["source_grounded_news_fallback", "source_depth_verified"],
      metrics: {
        sourcePoints: points.length,
        sourceWords: sourceWordCount,
        factualPoints: factPoints.length,
