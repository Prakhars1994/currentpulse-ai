...
async function callGemini(model, prompt) {
  ...
  console.log(
    `[Article generation] Sending request to ${model}`
  );
  ...
  console.log(
    `[Article generation] Response received from ${model}`
  );
  ...
  for (const model of MODELS) {
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        const retryable =
          isTemporaryError(error) || isOutputError(error);
...
console.log("Starting Editorial Review...");
