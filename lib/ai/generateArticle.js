...
function validateArticle(article, mode = "upsc") {
  ...
  const normalizedArticle = {
    ...
    category: normalizeCategory(cleanText(article.category)),
    paper: normalizePaper(cleanText(article.paper)),
  ...
  const minimumLengths = mode === "news"
    ...
    : mode === "trusted"
      ? {
          ...
          syllabus_linkage: 24,
          india_relevance: 25,
          static_foundation: 90,
          data_examples: 90,
          prelims: 90,
          mains: 220,
          answer_framework: 100,
          question: 20,
          visual_summary: 30,
        }
      : {
  ...
  const qualityMode = mode === "trusted" ? "upsc" : mode;
  const quality = assessArticleQuality(normalizedArticle, { mode: qualityMode });
  const trustedCriticalFlags = new Set([
    "article_too_short",
    "thin_mains",
    "weak_exam_utility",
    "editorial_residue",
    "repetitive_sections",
    "insufficient_revision_points",
  ]);
  const qualityAccepted =
    mode === "trusted"
      ? Number(quality.score || 0) >= 72 &&
        !quality.flags.some((flag) => trustedCriticalFlags.has(flag))
      : quality.passed;
  if (!qualityAccepted) {
    throw new Error(
...
async function generateWithFallback(prompt, stageName, mode = "upsc", options = {}) {
  let lastError;
  const automated = options.automated === true;
  const models = automated ? MODELS.slice(0, 2) : MODELS;
  const attemptsPerModel = automated ? 1 : 2;
...
9. Never echo source-bundle labels such as "SOURCE TITLE", "SOURCE URL",
   "ADDITIONAL COVERAGE", "OFFICIAL VERIFICATION" or
   "COMPLETE EXTRACTED SOURCE CONTENT" into reader-facing fields.
10. What happened, Key facts, Context and Why it matters must contain distinct
    information. Never reuse the same paragraph across multiple fields.
11. If retained sources disagree on a number, date, magnitude or other material
    fact, do not silently concatenate both claims. Attribute the disagreement,
    prefer a latest/authoritative value only when the supplied material supports
    that choice, and omit a disputed value when it cannot be reconciled.
...
export async function generateArticle(sourceContent, options = {}) {
  ...
  const validationMode = trustedCoverage ? "trusted" : newsMode ? "news" : "upsc";
  ...
  */
  const automatedSourceLimit = Math.max(
    9_000,
    Math.min(
      18_000,
      Number(process.env.AI_AUTOMATION_SOURCE_CHAR_LIMIT) || 14_000
    )
  );
  const limitedSource = cleanedSource.slice(
    0,
    trustedCoverage || newsMode ? automatedSourceLimit : 30_000
  );
    ...
    draftArticle = await generateWithFallback(
      ...
      trustedCoverage
        ...
        : newsMode
          ...
          : "Article generation",
      validationMode,
      { automated: trustedCoverage || newsMode }
    );
  } catch (error) {
    const message = String(error?.message || "").toLowerCase();
