...
function markProviderCoolingDown(id) {
...
async function generateWithCompatibleProvider(provider, contents, config = {}) {
  const controller = new AbortController();
  const timeoutMs = Math.max(15_000, Number(process.env.AI_PROVIDER_TIMEOUT_MS) || DEFAULT_TIMEOUT_MS);
...
export async function generateWithRouter({ model, contents, config = {} }) {
  ...
  if (gemini && !providerIsCoolingDown(`gemini:${model}`)) {
    try {
      ...
      if (isTemporaryProviderError(error)) markProviderCoolingDown(`gemini:${model}`);
  ...
  for (const definition of providerOrder()) {
    ...
    try {
      const result = await generateWithCompatibleProvider(provider, contents, config);
      ...
      if (isTemporaryProviderError(error)) markProviderCoolingDown(provider.id);
