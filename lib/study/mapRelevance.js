...
export function filterRelevantMapLocations({ title = "", category = "", text = "", mapLocations = [] } = {}) {
  ...
  const combined = clean(`${title} ${text}`);
  // Never let a generic India centroid become a substitute for the actual
  // foreign/place focus. India may still appear alongside a bilateral location
  // when it is explicitly useful.
  const nonIndia = locations.filter((location) => !/^india$/i.test(location));
  if (
    nonIndia.length &&
    /\b(?:international|bilateral|foreign|border|conflict|strait|sea|island|country|neighbour|neighbor)\b/i.test(combined)
  ) {
    const includesIndia = locations.some((location) => /^india$/i.test(location));
    return [...nonIndia, ...(includesIndia ? ["India"] : [])].slice(0, 6);
  }
