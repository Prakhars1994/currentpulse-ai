...
function classifyType(title = "") {
  ...
  if (/\b(admit\s*card|call\s*letter|e-?call\s*letter|city intimation)\b/.test(t)) return "admit-card";
...
async function fetchSource(source, state = null) {
  ...
  try {
    const headers = {
      "User-Agent": "CurrentPulse-ResultPulse/1.0 (+https://currentpulse-ai.vercel.app/exams)",
...
export async function collectOfficialExamUpdates(
  ...
  const sourceResults = await Promise.all(
    selectedSources.map(async (source) => {
      ...
      const previousState = sourceStates.get(key) || null;
