...
async function ttlCached(key,loader,ttlMs=CACHE_TTL_MS){const now=Date.now(),cached=cache.get(key);if(cached&&cached.expiresAt>now)return cached.value;const value=await loader();cache.set(key,{value,expiresAt:now+ttlMs});if(cache.size>80)for(const[k,e]of cache)if(e.expiresAt<=now)cache.delete(k);return value;}
