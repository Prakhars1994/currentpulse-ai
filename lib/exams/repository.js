...
function newestFirst(rows = []) {
  return [...rows].sort((left, right) => effectiveTime(right) - effectiveTime(left));
}

const cachedLatestExamUpdates = unstable_cache(
  async (type = "", limit = 24) => {
    const requestedLimit = Math.max(1, Math.min(Number(limit) || 24, 60));
    let query = supabase
      .from("exam_updates")
    ...
    return {
      updates: newestFirst(data || []).slice(0, requestedLimit),
      error: cleanError(error),
    };
  },
  ["resultpulse-latest-updates-v2"],
  { revalidate: false, tags: ["resultpulse-exams"] }
);
...
export const loadRelatedExamUpdates = unstable_cache(
  async (examName, excludeId = 0, limit = 12) => {
    ...
    const requestedLimit = Math.max(1, Math.min(Number(limit) || 12, 20));
    let query = supabase
      .from("exam_updates")
      .select(EXAM_CARD_FIELDS)
      .eq("status", "published")
  ...
  ["resultpulse-related-updates-v2"],
  { revalidate: 120, tags: ["resultpulse-exams"] }
);
