...
export async function loadExamUpdates({ type = "", group = "", source = "", q = "", limit = 24 } = {}) {
  ...
  const isFiltered = Boolean(active.type || active.group || sourceFilter || active.q);
  const candidateLimit = isFiltered ? Math.min(80, Math.max(safeLimit * 2, 30)) : 120;
  const cacheKey = `latest:${active.type}:${active.group}:${active.source}:${active.q}:${safeLimit}`;
