...
import { assessExamSitemapRecord, examDisplayTitle, selectExamSitemapRecords } from "@/lib/sitemapQuality";
...
const BALANCED_CACHE_TTL_MS = 600_000;
...
function publicRows(rows = []) {
  const normalized = rows.map(normalizeRow).filter((row) => assessExamSitemapRecord(row).allowed);
  return selectExamSitemapRecords(normalized).included;
}

// Only an authority-supplied date is a trustworthy freshness signal. created_at
// and updated_at describe our ingestion lifecycle and must never make an old
// authority page look like a newly issued exam update.
...
async function loadBalancedFrontPage(limit) {
  return ttlCached(`balanced-front:${limit}`, () => safeQuery(async () => {
    const supabase = createServerSupabase();
    const perSource = Math.max(3, Math.ceil(limit / ACTIVE_PUBLIC_SOURCES.length) + 1);
    ...
    return { updates: rank(publicRows(results.flat())).slice(0, limit), hasMore: true, error: null };
...
export async function loadExamUpdates({ type = "", group = "", source = "", q = "", limit = 24, offset = 0 } = {}) {
  ...
  return ttlCached(cacheKey, () => safeQuery(async () => {
    const supabase = createServerSupabase();
    let query = supabase.from("exam_updates").select(FIELDS).eq("status", "published").not("source_published_at", "is", null).order("source_published_at", { ascending: false });
    ...
    const rows = data || [];
    return { updates: rank(publicRows(rows.slice(0, safeLimit))), hasMore: rows.length > safeLimit, error: cleanError(error) };
  }));
}
...
export async function loadExamUpdateBySlug(slug) {
  ...
  return ttlCached(`slug:${slug}`, () => safeQuery(async () => {
    const supabase = createServerSupabase();
    const { data, error } = await supabase.from("exam_updates").select("*").eq("slug", slug).eq("status", "published").not("source_published_at", "is", null).maybeSingle();
...
export async function loadRelatedExamUpdates(examName, excludeId = 0, limit = 12) {
  ...
  return ttlCached(`related:${examName}:${excludeId}:${safeLimit}`, () => safeQuery(async () => {
    const supabase = createServerSupabase();
    let query = supabase.from("exam_updates").select(FIELDS).eq("status", "published").not("source_published_at", "is", null).eq("exam_name", examName).order("source_published_at", { ascending: false }).limit(40);
    if (excludeId) query = query.neq("id", excludeId);
    const { data, error } = await query;
    return { updates: rank(publicRows(data || [])).slice(0, safeLimit), error: cleanError(error) };
  }));
}
