...
function normalizeRow(row = {}) {
  return { ...row, title: normalizeTitle(row.title) || row.exam_name || "Official exam update", summary: String(row.summary || "").replace(/\s+/g, " ").trim() };
}
...
async function loadBalancedFrontPage(limit) {
  return ttlCached(`balanced-front:${limit}`, () => safeQuery(async () => {
    ...
    return { updates: rank(results.flat()).slice(0, limit), hasMore: true, error: null };
...
export async function loadExamUpdates({ type = "", group = "", source = "", q = "", limit = 24, offset = 0 } = {}) {
  ...
  return ttlCached(cacheKey, () => safeQuery(async () => {
    ...
    const rows = data || [];
    return { updates: rank(rows.slice(0, safeLimit)), hasMore: rows.length > safeLimit, error: cleanError(error) };
  }));
}
