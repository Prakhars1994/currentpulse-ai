...
const TAXONOMY_OVERRIDES = [
  {
    pattern: /\b(?:epigenetic|epigenome|dna methylation|histone modification|gene expression|genetic inheritance|genomics?)\b/i,
    category: "Science & Technology",
    paper: "GS-3",
  },
  {
    pattern: /\b(?:nauru|naoero)\b[\s\S]{0,220}\b(?:rename|renamed|name change|official name|republic|pacific island|diplomatic|foreign policy)\b|\b(?:rename|renamed|name change|official name|republic|pacific island|diplomatic|foreign policy)\b[\s\S]{0,220}\b(?:nauru|naoero)\b/i,
...
export function correctTaxonomy(text, suggestedCategory = "", suggestedPaper = "") {
  ...
  return {
    category,
    paper: resolvePaper(category, suggestedPaper),
    overridden: false,
  };
}
