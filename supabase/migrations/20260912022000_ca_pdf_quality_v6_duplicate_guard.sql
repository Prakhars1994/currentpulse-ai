-- CA PDF quality contract v6: keep the v5 structure/source rules and reject exact
-- duplication between FAST READ and WHY IN NEWS. The public reader still receives
-- the internal CA_TITLE marker so canonical PDF markdown uses the strict renderer.
...
as $$
declare
 c text:=coalesce(body,''); fast text:=''; why text:=''; facts text:=''; pre text:=''; quick text:=''; model text:=''; sources text:='';
 facts_bullets int:=0; pre_bullets int:=0; model_words int:=0; f text[]:=array[]::text[];
 fast_key text:=''; why_key text:='';
begin
 c:=replace(c,E'\\n',E'\n');
 ...
 if position('## FAST READ' in c)>0 and position('## WHY IN NEWS' in c)>0 then
   fast:=split_part(split_part(c,'## FAST READ',2),'## WHY IN NEWS',1);
   why:=split_part(split_part(c,'## WHY IN NEWS',2),'## TOP DATA & FACTS',1);
   fast_key:=regexp_replace(lower(fast),'[^[:alnum:]]+','','g');
   why_key:=regexp_replace(lower(why),'[^[:alnum:]]+','','g');
   if length(fast_key)>=80 and fast_key=why_key then f:=array_append(f,'FAST/WHY DUPLICATE'); end if;
 end if;
