...
function cleanNewsText(v=""){
 return String(v||"")
  .replace(/\[\[NEWS_(?:START|END)\]\]/gi," ")
  .replace(/\bNEWS_(?:TITLE|SCOPE|SECTION|DATE|STYLE|IMAGE)\s*:\s*[^\n\r]*/gi," ")
  .replace(/\bPART\s+\d+\s*\|\s*NEWS\s+\d+\s*[-–]\s*\d+\s*\|[^\n\r]*/gi," ")
  .replace(/\bCURRENTPULSE\s+NEWS\b/gi," ")
  .replace(/<[^>]*>/g," ").replace(/&nbsp;/g," ").replace(/&amp;/g,"&")
  .replace(/\*\*([^*]+)\*\*/g,"$1").replace(/__([^_]+)__/g,"$1").replace(/`([^`]+)`/g,"$1")
  .replace(/(^|\s)[#>*_~-]+(?=\S)/g,"$1").replace(/\s+/g," ").trim();
}
function storyDek(article,limit=220){const raw=cleanNewsText(article?.why_news||""); const title=cleanNewsText(article?.title||""); let text=raw; if(title&&text.toLowerCase().startsWith(title.toLowerCase())) text=text.slice(title.length).replace(/^\s*[-:|–—]+\s*/,""); return text.slice(0,limit).trim();}
...
export default async function NewsPage({searchParams}){
 ...
 return <main className="newsroom-page min-h-screen"><div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-10">
   ...
   {lead&&<section className="news-front-grid"><article className="news-front-lead">{resolveDisplayImage(lead)&&<Link href={`/news/${lead.slug}`} className="news-front-lead-image"><img src={resolveDisplayImage(lead)} alt={lead.title}/></Link>}<div className="news-front-label">TOP STORY · {lead.category||"NEWS"}</div><Link href={`/news/${lead.slug}`}><h2>{cleanNewsText(lead.title)}</h2></Link><p>{storyDek(lead,360)||"Open the story for the latest verified details."}</p><div className="news-front-byline"><span>CurrentPulse Newsroom</span><time>{formatDate(lead.created_at)}</time><Link href={`/news/${lead.slug}`}>Continue reading →</Link></div></article>
     <aside className="news-front-rail">{secondary.map((a,i)=><article key={a.id}>{resolveDisplayImage(a)&&<Link href={`/news/${a.slug}`} className="news-rail-image"><img src={resolveDisplayImage(a)} alt={a.title}/></Link>}<div><small>{i===0?"BREAKING / LEAD":"TOP STORY"} · {a.category||"News"}</small><Link href={`/news/${a.slug}`}><h3>{cleanNewsText(a.title)}</h3></Link><p>{storyDek(a,130)}</p><time>{formatDate(a.created_at)}</time></div></article>)}</aside>
   </section>}
   <div className="news-section-rule"><span>LATEST EDITION</span><p>Chronological archive · source-attributed reporting</p></div>
   {archive.length?<section className="news-paper-grid">{archive.map((article,index)=>{const image=resolveDisplayImage(article);const dateLabel=formatDate(article.created_at);const prev=index>0?formatDate(archive[index-1]?.created_at):"";return <Fragment key={article.id}>{(index===0||dateLabel!==prev)&&<div className="news-date-divider"><span>{dateLabel}</span></div>}<article className={`news-paper-card ${index%7===0?"news-paper-card--feature":""}`}>{image&&<Link href={`/news/${article.slug}`} className="news-paper-card-image"><img src={image} alt={article.title} loading="lazy"/></Link>}<div className="news-paper-card-copy"><div className="news-paper-meta"><Link href={`/category/${createCategorySlug(article.category)}`}>{article.category||"News"}</Link><time>{dateLabel}</time></div><Link href={`/news/${article.slug}`}><h2>{cleanNewsText(article.title)}</h2></Link><p>{storyDek(article,220)||"Open the story for verified details and context."}</p><Link href={`/news/${article.slug}`} className="news-paper-read">Full story →</Link></div></article></Fragment>})}</section>:!lead&&<div className="newsroom-empty"><h2>No stories on this page</h2><p>Published News stories will appear here.</p></div>}
