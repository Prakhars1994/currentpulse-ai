...
}, ["currentpulse-news-detail-v3"], { revalidate: 60, tags: ["currentpulse-articles", "currentpulse-news"] });
...
export default async function NewsArticlePage({ params }) {
  ...
  const newsPresentation=parseNewsPresentation(article.content);const title=newsPresentation?.title||repairedNewsTitle(article);const newsLead=cleanNewsSection(newsPresentation?.lead||article.why_news,["What happened","Why in News","The development"]);const category=normalizedPublicCategory(article.category,`${title} ${newsLead||article.why_news||""}`);const newsFacts=cleanNewsSection(newsPresentation?.keyFacts||article.data_examples,["Key facts","At a glance","Data, Reports, Cases & Examples"]);const newsContext=cleanNewsSection(newsPresentation?.context||article.static_foundation,["Context","Background","Static Foundation"]);const newsWhyItMatters=cleanNewsSection(newsPresentation?.whyItMatters||article.india_relevance,["Why it matters","Significance","India relevance"]);const hasCurrentAffairsView=hasCoachingSource;const image=resolveDisplayImage(article);const verifiedReusableImage=isVerifiedReusableArticleImage(article);const canonical=`${SITE_URL}/news/${slug}`;const licensedConversation=Array.isArray(article.quality_flags)&&article.quality_flags.includes("licensed_republish_the_conversation");if(licensedConversation)return <LicensedNewsArticle article={article} sources={sources}/>;
  const structuredData={"@context":"https://schema.org","@type":"NewsArticle",headline:title,description:stripHtml(newsLead),datePublished:article.created_at,dateModified:article.updated_at||article.created_at,mainEntityOfPage:canonical,...(image?{image:[absoluteSiteUrl(image)]}:{}),author:{"@type":"Organization",name:"CurrentPulse Newsroom",url:SITE_URL},publisher:{"@type":"Organization",name:"CurrentPulse AI",url:SITE_URL},citation:newsSources.map((source)=>source.source_url).filter(Boolean)};
  return <><ArticleViewTracker slug={slug}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structuredData)}}/><main className="news-article-page min-h-screen"><article className="news-article-shell"><nav className="news-article-breadcrumb"><Link href="/">Home</Link> / <Link href="/news">News</Link> / <span>{category}</span></nav><p className="news-article-category">{category||"Latest News"}</p><h1>{title}</h1><div className="news-article-byline"><span>CurrentPulse Newsroom</span><time>Published {formatDate(article.created_at)}</time>{article.updated_at!==article.created_at&&<time>Updated {formatDate(article.updated_at)}</time>}</div>{hasCurrentAffairsView&&<div className="news-upsc-bridge"><span>Preparing for UPSC?</span><Link href={`/current-affairs/${slug}`}>Open the Current Affairs analysis →</Link></div>}
  {image&&<figure className="news-article-figure"><img src={image} alt={article.image_alt||title}/>{verifiedReusableImage&&article.image_caption&&<figcaption>{article.image_caption}</figcaption>}</figure>}
  <section className="news-article-lead"><div className="news-section-kicker">The development</div><h2>What happened</h2><ArticleContent content={newsLead}/></section>
  <ArticleStudyVisuals mapLocations={article.map_locations} title={title} articleText={`${newsLead||""} ${newsFacts||""} ${newsContext||""}`} category={category}/>
  {newsFacts&&<section className="news-article-section news-article-facts"><div className="news-section-kicker">At a glance</div><h2>Key facts</h2><EvidenceHighlights content={newsFacts} limit={5}/></section>}
  {newsContext&&<section className="news-article-section"><div className="news-section-kicker">Background</div><h2>Context</h2><ArticleContent content={newsContext}/></section>}
  {newsWhyItMatters&&<section className="news-article-section news-article-why"><div className="news-section-kicker">Significance</div><h2>Why it matters</h2><ArticleContent content={newsWhyItMatters}/></section>}
  ...
  <div className="news-article-footer"><Link href="/news">← Back to latest news</Link></div></article></main></>;
}
