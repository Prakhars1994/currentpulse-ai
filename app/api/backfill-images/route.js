...
async function executeBackfill(limit,targetStream="news"){
  ...
  // Only legacy/unprocessed rows with no recorded image decision are eligible.
  // Resolved and terminal no-safe-image rows never need to be scanned again.
  const{data,error}=await supabase.from("articles").select("id,title,slug,category,why_news,content,static_foundation,quality_flags,image,image_url,image_source_url,image_caption,image_search_query,image_resolution,created_at,article_sources(source_kind,source_url,source_name)").eq("status","published").is("image_resolution",null).order("created_at",{ascending:false}).limit(SCAN_LIMIT);if(error)throw new Error(`Image backfill fetch failed: ${error.message}`);
