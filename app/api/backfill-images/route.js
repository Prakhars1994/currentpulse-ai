...
async function isAuthorised(request) {
  ...
  try { const auth = await requireAuthenticatedAdmin(request); return Boolean(auth?.ok); } catch { return false; }
}
...
async function executeBackfill(limit, targetStream = "news") {
  ...
  const needsReplacement=(article)=>{const stream=(article.article_sources||[]).some((source)=>source?.source_kind==="coaching")?"coverage":"news";const safeForImage=stream==="news"?true:isPublishedArticleSafe(article,{stream});return stream===targetStream&&safeForImage&&!isVerifiedReusableArticleImage(article)&&!isTerminalImageResolution(article.image_resolution);};
