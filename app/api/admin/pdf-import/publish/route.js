...
function buildArticlePayload(article, { stream, date, fileHash } = {}) {
  ...
  return { title, slug, category, paper, why_news: whyNews || fullText.slice(0, 900), prelims, mains, question, content: fullText, static_foundation: staticFoundation, data_examples: dataExamples, india_relevance: indiaRelevance, syllabus_linkage: stream === "ca" || stream === "ca_hi" ? `- **Paper:** ${paper}\n- **Theme:** ${category}` : "", seo_title: clean(article.seo_title || title).slice(0, 180), seo_description: clean(article.seo_description || whyNews || fullText).slice(0, 160), tags: [...new Set((Array.isArray(article.tags) ? article.tags : [category, paper, "PDF Import"]).map(clean).filter(Boolean).slice(0, 16))], status: "published", language: stream === "ca_hi" ? "hi" : "en", created_at: createdAt, updated_at: createdAt, published_at: `${date}T12:00:00`, image_alt: title, image_search_query: title, image_url: imageUrl || null, image_source_url: imageSourceUrl || null, image_caption: imageCaption || null, image_resolution: reviewedManualImage ? { status: "preserved_existing", provider: "manual_license_review", requests_used: 0, attribution: imageCaption, source_page_url: imageSourceUrl, license_or_usage: "manual_license_review", search_query: title } : { status: "deferred_after_pdf_publish", provider: "deferred", requests_used: 0 }, map_locations: mapLocations, quality_score: 100, quality_version: 6, quality_flags: ["admin_pdf_import","zero_ai_pdf_import","full_text_preserved","structure_validated",...(reviewedManualImage ? ["manual_image_license_review"] : ["image_resolution_deferred"]),stream === "ca" || stream === "ca_hi" ? "ca_pdf_import" : "news_pdf_import",...(stream === "ca_hi" ? ["hindi_ca_pdf_import"] : [])], manual_protected: true };
}

// Kept as the shared resolve-once implementation for explicit image/backfill flows.
// The batch publish path below deliberately does NOT call it: image lookup can issue
// multiple external fetches per article and was exhausting Cloudflare subrequests
// before all selected PDF articles and the reader refresh were committed.
function eventShape(article = {}) {
  return { title: article.title || "", description: article.why_news || "", published_at: article.published_at || article.created_at || null };
}

async function loadReusableImageBank(supabase) {
  const { data, error } = await supabase.from("articles").select("id,title,category,why_news,published_at,created_at,image,image_url,image_alt,image_caption,image_source_url,image_search_query,image_resolution").eq("status", "published").not("image_url", "is", null).order("published_at", { ascending: false, nullsFirst: false }).limit(IMAGE_BANK_LIMIT);
  if (error) { console.error("PDF reusable image bank lookup failed:", error.message); return []; }
...
function reusableImagePatch(candidate, payload) {
  ...
  return { image: imageUrl, image_url: imageUrl, image_alt: candidate.image_alt || payload.title, image_caption: candidate.image_caption || null, image_source_url: candidate.image_source_url || null, image_search_query: payload.image_search_query || payload.title, image_resolution: { status: "preserved_existing", provider: candidate.image_resolution?.provider || "existing_article", source_article_id: candidate.id, requests_used: 0, search_query: payload.image_search_query || payload.title }, updated_at: new Date().toISOString() };
}
...
export async function POST(request) {
  ...
  try {
    ...
    const stream = body?.stream === "news" ? "news" : body?.stream === "ca_hi" ? "ca_hi" : body?.stream === "ca" ? "ca" : "";
    const fileName = clean(body?.fileName).slice(0, 220); const fileHash = clean(body?.fileHash).toLowerCase(); const date = safeDate(body?.publishedAt); const articles = Array.isArray(body?.articles) ? body.articles : [];
    ...
    const normalized = articles.map((article, batchIndex) => {
      const importIndex = Number(article?.importIndex); const title = clean(article?.title); const fullText = preserveText(article?.fullText);
      if (!Number.isInteger(importIndex) || importIndex < 0 || title.length < 5 || fullText.trim().length < 80) throw new Error(`Article ${batchIndex + 1} is missing a valid title, index or body.`);
      return { sourceKey: `pdf:${stream}:${fileHash}:${importIndex}`, importIndex, article };
    });

    const { data: existingSources, error: existingError } = await auth.supabase.from("article_sources").select("source_key,article_id").in("source_key", normalized.map((item) => item.sourceKey));
    if (existingError) throw new Error(`PDF duplicate lookup failed: ${existingError.message}`);
    ...
    for (const item of normalized) {
      ...
      if (existing.has(item.sourceKey)) {
        results.push({ status: "duplicate", importIndex: item.importIndex, articleId: existing.get(item.sourceKey), title: payload.title, image: { status: "unchanged_or_deferred", requestsUsed: 0 } });
        continue;
      }

      const { data: inserted, error: insertError } = await auth.supabase.from("articles").insert([payload]).select("id,slug,title").single();
      if (insertError) { results.push({ status: "failed", importIndex: item.importIndex, title: payload.title, error: insertError.message }); continue; }

      const isCurrentAffairs = stream === "ca" || stream === "ca_hi";
      const sourceName = isCurrentAffairs ? "CurrentPulse Admin CA PDF" : "CurrentPulse Admin News PDF";
      const sourceUrl = isCurrentAffairs ? `${SITE_URL}${stream === "ca_hi" ? "/current-affairs?lang=hi" : "/current-affairs"}` : `${SITE_URL}/news`;
      ...
      if (sourceError) {
        ...
        results.push({ status: "failed", importIndex: item.importIndex, title: payload.title, error: `Source registration failed: ${sourceError.message}` });
        continue;
      }
      results.push({ status: "published", importIndex: item.importIndex, articleId: inserted.id, slug: inserted.slug, title: inserted.title, image: { status: payload.image_url ? "provided" : "deferred", requestsUsed: 0 } });
    }
    ...
    if (published > 0) {
      try {
        ...
        readerRefreshWarning = "Articles are published in the database, but the immediate reader refresh could not be dispatched.";
