...
export async function generateMetadata({ params }) {
  ...
  const description = String(update.summary || `Official ${update.update_type} update from ${update.source_name || update.agency}. Verify details at the issuing authority.`).slice(0, 160);
...
export default async function ExamDetail({ params }) {
  ...
  const displayTitle = resultPulseDisplayTitle(update);
  const canonical = `${SITE_URL}/exams/${slug}`;
  const structuredData = { "@context": "https://schema.org", "@type": "WebPage", name: displayTitle, description: update.summary || undefined, url: canonical, datePublished: update.source_published_at || update.created_at, dateModified: update.updated_at || update.created_at, mainEntityOfPage: canonical, isBasedOn: update.official_url, about: { "@type": "Thing", name: update.exam_name || displayTitle }, publisher: { "@type": "Organization", name: "CurrentPulse AI", url: SITE_URL } };
  const breadcrumbData = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: SITE_URL },
    { "@type": "ListItem", position: 2, name: "ResultPulse AI", item: `${SITE_URL}/exams` },
    { "@type": "ListItem", position: 3, name: displayTitle, item: canonical },
  ] };
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structuredData)}}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(breadcrumbData)}}/><main className="min-h-screen bg-slate-950 py-10 text-white sm:py-14"><article className="mx-auto max-w-4xl px-4 sm:px-6">
    <nav className="text-sm font-bold text-slate-500"><Link href="/">Home</Link> / <Link href="/exams">Exams</Link> / {meta.label}</nav>
    <div className="mt-6 rounded-[2rem] border border-violet-400/20 bg-slate-900/85 p-7 shadow-2xl shadow-slate-950/30 sm:p-10"><span className="rounded-full bg-violet-400/10 px-3 py-1.5 text-xs font-black uppercase text-violet-300">{meta.icon} {meta.label}</span><h1 className="mt-5 text-3xl font-black leading-tight sm:text-5xl">{displayTitle}</h1><div className="mt-5 flex flex-wrap gap-3 text-sm text-slate-400"><strong className="text-slate-200">{update.agency || update.source_name}</strong>{(update.source_published_at || update.created_at) && <span>Updated {formatDate(update.source_published_at || update.created_at)}</span>}</div>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-800 p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Exam / recruitment</p><p className="mt-2 font-black">{update.exam_name || displayTitle}</p></div><div className="rounded-2xl border border-slate-800 p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Authority</p><p className="mt-2 font-black">{update.agency || update.source_name}</p></div></div>
