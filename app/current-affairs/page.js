...
function stripHtml(content = "") { return String(content || "").replace(/<[^>]*>/g," ").replace(/&nbsp;/g," ").replace(/&amp;/g,"&").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/\s+/g," ").trim(); }
function pageHref(page, todayOnly, exam="upsc", hi=false) { const p=new URLSearchParams(); if(page>1)p.set("page",String(page)); if(todayOnly)p.set("view","today"); if(exam!=="upsc")p.set("exam",exam); if(hi)p.set("lang","hi"); return p.toString()?`/current-affairs?${p}`:"/current-affairs"; }
function dateLabel(value){ if(!value)return ""; return new Date(value).toLocaleDateString("en-IN",{timeZone:"Asia/Kolkata",day:"numeric",month:"short",year:"numeric"}); }

export default async function CurrentAffairsPage({ searchParams }) {
  const params=await searchParams; const requestedPage=Math.max(1,Number(params?.page)||1); const todayOnly=params?.view==="today"; const hi=params?.lang==="hi"; const exam=getExamVertical(params?.exam||"upsc");
  const pageSize=24, offset=(requestedPage-1)*pageSize;
  ...
  const totalPages=Number.isFinite(total)?Math.max(1,Math.ceil(total/pageSize)):null; const articleSuffix=hi?"?lang=hi":""; const lead=articles?.[0]; const rest=articles?.slice(1)||[];
  ...
  return <main className="ca-edition min-h-screen">
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      {lead ? <>
        <section className="ca-lead-story">
          <div className="ca-lead-copy"><div className="ca-story-meta"><span>{lead.category||"Current Affairs"}</span><b>{lead.paper||"General Studies"}</b><time>{dateLabel(lead.created_at)}</time></div><Link href={`/current-affairs/${lead.slug}${articleSuffix}`}><h2>{lead.title}</h2></Link><p>{stripHtml(lead.why_news)||"Open the brief for complete exam-oriented analysis."}</p><div className="ca-lead-footer"><small>{currentAffairsSourceLabel(lead)}</small><Link href={`/current-affairs/${lead.slug}${articleSuffix}`}>Study this brief →</Link></div></div>
          {resolveDisplayImage(lead)?<Link className="ca-lead-image" href={`/current-affairs/${lead.slug}${articleSuffix}`}><img src={resolveDisplayImage(lead)} alt={lead.title}/></Link>:<div className="ca-lead-visual"><span>DAILY BRIEF</span><strong>{lead.category||"UPSC"}</strong></div>}
        </section>
        <div className="ca-section-rule"><span>MORE CURRENT AFFAIRS</span></div>
        <section className="ca-editorial-grid">{rest.map((article,index)=>{const image=resolveDisplayImage(article); const href=`/current-affairs/${article.slug}${articleSuffix}`; return <article key={article.id} className={`ca-editorial-card ${index<2?"ca-editorial-card--wide":""}`}>
          {image&&<Link href={href} className="ca-editorial-image"><img src={image} alt={article.title} loading="lazy"/></Link>}
          <div className="ca-editorial-body"><div className="ca-story-meta"><Link href={`/category/${createCategorySlug(article.category)}${articleSuffix}`}>{article.category||"Current Affairs"}</Link><b>{article.paper||"GS"}</b><time>{dateLabel(article.created_at)}</time></div><Link href={href}><h2>{article.title}</h2></Link><p>{stripHtml(article.why_news)||"Read the complete current affairs analysis."}</p><div className="ca-card-foot"><small>{currentAffairsSourceLabel(article)}</small><Link href={href}>Open brief →</Link></div></div>
        </article>})}</section>
