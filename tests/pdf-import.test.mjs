...
function page(pageNumber, lines) { return { pageNumber, width: 600, height: 800, lines }; }
...
test("PDF extraction repairs only deterministic glyph artifacts", () => {
  ...
  assert.equal(normalizePdfExtractionArtifacts(source), "India has the 2nd-largest network. Action Plans improve logistics and bringing. GI tags remain protected.");
});
...
test("PDF importer exposes separate English CA, Hindi CA and News PDF lanes", () => {
  ...
  for (const marker of ['stream="ca"','stream="ca_hi"','stream="news"','Current Affairs PDF','हिंदी Current Affairs PDF','News PDF','No AI call is used for extraction']) assert.ok(workspace.includes(marker));
...
test("PDF lanes use visible chooser labels connected to separate hidden file inputs", () => {
  ...
  for (const marker of ["Choose PDF",'className="sr-only"',"sticky top-4 z-20","Upload a PDF","No PDF selected","images.google.com/advanced_search","image_source_url","image_caption"]) assert.ok(workspace.includes(marker));
...
test("PDF publish route records English CA, Hindi CA and News source rows", () => {
  ...
  for (const pattern of [/MAX_ARTICLES_PER_REQUEST = 20/,/stream === "ca" \|\| stream === "ca_hi"/,/CurrentPulse Admin CA PDF/,/CurrentPulse Admin News PDF/,/zero_ai_pdf_import/,/full_text_preserved/,/releaseRequired: releaseRows\.length > 0/,/language: stream === "ca_hi" \? "hi" : "en"/,/map_locations: mapLocations/,/image_url: imageUrl \|\| null/]) assert.match(route, pattern);
});
...
test("CA admin PDF stays an internal policy exception", () => {
  ...
  const ids = [...sourcePolicy.matchAll(/id:\s*"([^"]+)"/g)].map((m) => m[1]);
  assert.deepEqual(ids.slice(0,7), ["vision","drishti","insights","forum","nextias","vajiram","iasbaba"]);
});

test("PDF front matter cannot become article titles", () => {
  const importer = load("lib/pdf/importFormat.js").toLowerCase();
  const safety = load("lib/editorial/publicationSafety.js").toLowerCase();
  for (const marker of ["currentpulse\\s+ai","today['’]?s","how\\s+to\\s+use","topic\\s+mix","related\\s+upsc"]) {
    assert.ok(importer.includes(marker.toLowerCase()));
    assert.ok(safety.includes(marker.toLowerCase()));
  }
});
...
test("synthetic PDF headings become separate CA articles", async () => {
  ...
  const tempPath = path.resolve(process.cwd(), "tests/.tmp-pdf-import-format.mjs");
  ...
  try {
    ...
    const preview = buildPdfImportPreview({ stream:"ca", fileName:"daily-ca.pdf", fileHash:"a".repeat(64), pages:[
      page(1,[{text:"CurrentPulse Daily",fontSize:9,bold:false,y:780},{text:"India-EU Trade Agreement",fontSize:18,bold:true,y:700},{text:"Why in News",fontSize:11,bold:true,y:660},{text:"India and the European Union announced a new trade milestone.",fontSize:10,bold:false,y:630},{text:"Prelims",fontSize:11,bold:true,y:590},{text:"The EU is a political and economic union of European states.",fontSize:10,bold:false,y:560},{text:"Page 1",fontSize:9,bold:false,y:20}]),
      page(2,[{text:"CurrentPulse Daily",fontSize:9,bold:false,y:780},{text:"New Wetland Conservation Programme",fontSize:18,bold:true,y:700},{text:"Background",fontSize:11,bold:true,y:660},{text:"Wetlands support biodiversity, water regulation and local livelihoods.",fontSize:10,bold:false,y:630},{text:"Mains",fontSize:11,bold:true,y:590},{text:"Implementation depends on local institutions and ecological monitoring.",fontSize:10,bold:false,y:560},{text:"Page 2",fontSize:9,bold:false,y:20}]),
      page(3,[{text:"CurrentPulse Daily",fontSize:9,bold:false,y:780},{text:"Closing note for the wetland programme provides additional implementation detail and must remain attached.",fontSize:10,bold:false,y:640},{text:"Page 3",fontSize:9,bold:false,y:20}])
    ]});
...
test("strict markers are the only boundaries and fail closed", async () => {
  ...
  try {
    const { buildPdfImportPreview } = await import(`${pathToFileURL(tempPath).href}?v=${Date.now()}`);
    const lines=["[[CA_START]]","CA_TITLE: First Article","CA_CATEGORY: International Relations","CA_GS: GS Paper II","CA_DATE: 31 August 2026","CA_IMAGE: NO","Why in News","**important UPSC keyword**","## Way Forward","Probable Mains Question","[[CA_END]]","[[CA_START]]","CA_TITLE: Second Article","CA_CATEGORY: Environment","CA_GS: GS Paper III","CA_DATE: 31 August 2026","CA_IMAGE: YES","### Internal heading","body","[[CA_END]]","[[CA_START]]","CA_TITLE: Third Article","CA_CATEGORY: Economy","CA_GS: GS Paper III","CA_DATE: 31 August 2026","CA_IMAGE: https://example.com/a.png","1. Numbered section","body","[[CA_END]]"];
...
test("CA display headings are renderer-only and import safeguards remain intact", () => {
  ...
  for (const label of ["WHY IN NEWS","PRELIMS QUICK REVISION","WAY FORWARD","PROBABLE MAINS QUESTION","FAST READ","TOP DATA & FACTS","POLITICAL / GOVERNANCE PERSPECTIVE","PROBABLE OBJECTIVE QUESTION","PROBABLE DESCRIPTIVE QUESTION"]) assert.ok(renderer.includes(label),`Renderer is missing ${label}`);
  ...
  assert.match(renderer,/isStrictPdfSource/);
  assert.match(renderer,/normalizeStrictPdfMarkdown/);
  assert.match(renderer,/stripEmptyMarkdownHeadings/);
  assert.doesNotMatch(renderer,/<pre className="strict-pdf-verbatim">/);
  assert.match(load("lib/pdf/clientExtract.js"),/const hasMixedWeight = runs\.some/);
  assert.match(load("lib/pdf/clientExtract.js"),/\*\*\$\{run\.text\}\*\*/);
  assert.match(load("components/ArticleContent.css"),/background: #d9eaf7/);
