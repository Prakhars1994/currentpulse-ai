...
test("Conversation page fallback preserves exact body and required credits/counter", () => {
  ...
  const canonical =
    "https://theconversation.com/research-evidence-story-286999";
  const html = `
    <html><body>
  ...
  const fallback = buildConversationPageFallback(
    html,
    canonical,
    "286999"
  );
  ...
  assert.match(
    sanitized,
    /counter\.theconversation\.com\/content\/286999\/count\.gif/
  );
  assert.match(sanitized, /original article/);
});

test("automatic News is bounded and reuses a duplicate snapshot", () => {
  const route = load("app/api/auto-publish/route.js");
  const publisher = load("lib/publisher/publishArticle.js");

  assert.match(route, /NEWS_MAX_PUBLISH_PER_RUN/);
  assert.match(route, /evaluated\.recentArticles/);
  assert.match(route, /duplicateSnapshot:\s*recentArticles/);
...
test("public list paths no longer fetch full News/CA bodies", () => {
  ...
  const newsList = streams.match(
    /const NEWS_LIST_FIELDS_INNER = `([\s\S]*?)`;/m
  )?.[1] || "";
  const caList = streams.match(
    /const CURRENT_AFFAIRS_LIST_FIELDS_INNER = `([\s\S]*?)`;/m
  )?.[1] || "";
